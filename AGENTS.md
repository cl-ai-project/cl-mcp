...
A few of cl-mcp's own functions (`ensure-trailing-newline`, `sanitize-for-json`, `sanitize-error-message`) carry cl-spec contracts in the opt-in `cl-mcp/specs` system. When a change touches one of them, read its contract before editing and check it before and after, as `docs/specs.md` describes; `scripts/check-specs.lisp` runs the same checks CI does. Elsewhere the bundle is not required.
