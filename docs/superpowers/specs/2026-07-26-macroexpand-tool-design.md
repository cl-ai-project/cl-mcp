...
`*print-circle*` は**通常 nil** にして共有マーカーを排除する。バッククォート展開は
リテラルを日常的に共有するため、`#1=` が出ると結果が読みづらくなるからである。

**当初の設計は「ハング防止は `*print-level*` の有限束縛で担保できる」としていたが、
これは実測により誤りと判明した。** SBCL のプリティプリンタは `quote` 略記
（`SB-PRETTY::PPRINT-QUOTE`）で `*print-level*` を尊重せず、自己参照構造を渡すと
制御スタックを食い尽くして**プロセスごと落ちる**。`*print-pretty*` を nil にすれば
`*print-level*` は効くが、整形出力はこのツールの中心的な価値なので放棄できない。

したがって実際の方式は: **印字前に循環を検出し、循環している場合に限り
`*print-circle*` を t にする**。共有はあるが循環していない通常の展開結果では
nil のままなのでマーカーは出ず、病的な循環展開でもクラッシュせず
`#1='#1#` のように有限の出力になる。

検出は cons のみを辿る深さ優先走査で、訪問済みノードをメモ化するため、
共有の多い非循環 DAG でも重複走査は起きない（26万ノードの共有 DAG で
検出込み1秒未満を実測）。リテラルベクタや構造体を経由する循環は対象外だが、
マクロ展開でそれが起きることは実質的にない。

加えて `*print-level*` / `*print-length*` は有限値（既定 50 / 1000）で必ず束縛し、
`max_output_length` で最終的に切る。呼び出し側が `print_level` に `nil` を渡して
無制限にすることは許可しない。
