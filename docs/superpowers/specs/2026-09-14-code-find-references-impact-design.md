...
| なし | あり | `source` | トップレベル使用、未コンパイルのコード、ロード後に変更されたファイルのいずれか。候補が xref の通常記録しない種別（`quoted` / `template` / `method`）だけなら、xref に無いのは当然なので注記しない（例外として `WHO-CALLS` は `(mapcar 'name xs)` や `:key 'name` のように名前で渡した関数を記録するが、そのフォームはコンパイル済みなら xref にも現れ `source` だけにはならない） |
...
- **番号は行範囲で検証する**。親は自分の `*features*` でトップレベルフォームを数えるが、親は利用者の
  システムをロードしないので、コンパイルしたイメージと `*features*` が食い違うことがある（ロードされる
  システムが `eval-when` や `.asd` で `pushnew` する feature、worker にだけある feature）。すると SBCL が
  数えた `#+feature` のフォームを親は読み飛ばし、そのファイルの以降の番号が 1 つずつずれる。番号だけを
  キーにすると突き合わせは失敗するだけでなく**次のフォームに**当たり、呼び出し元を取り違える。そこで
  xref エントリの `line` が、同じ truename・同じ番号のフォームの `start_line`〜`end_line` に入るときだけ
  番号の一致を採る。入らなければ同じ truename で行範囲がその行を含むフォームに付け、それも無ければ
  突き合わせなし（下の `form-path` を持たないエントリと同じく行番号でまとめる）とする。キーは形を明示し、
  `(:form truename 番号)` と `(:line truename 行)` の 2 種類
- この検証は xref の `line` がそのエントリ自身の `(def...` の行であることを前提にする。`line` は worker の
  `%offset->line` が求める。バイト位置を文字位置に直した地点（直前のフォームの直後）から、空白と
  コメント（長さの上限なし、`#|...|#` は入れ子も数える）を読み飛ばし、`#+` / `#-` の feature 式を
  **worker の `*features*` で評価して**、偽ならそれが付いたフォームごと読み飛ばす（式は自前で解析して
  intern せず、解析できない式は真とみなす。フォームは `*read-suppress*` の `read` で飛ばす）。
  コンパイルしたリーダーと同じ判断で飛ばすので、`#+(or)`・`#-sbcl` のフォームや長いコメントの後ろでも、
  親だけが読んだフォームの後ろでも、`line` は正しいフォームを指す
