...
- **番号は行範囲で検証する**。親は自分の `*features*` でトップレベルフォームを数えるが、親は利用者の
  ...
  xref エントリの `line`（バイト位置を正しく変換した、トップレベルフォーム内の `(def...` の行）が、
  同じ truename・同じ番号のフォームの `start_line`〜`end_line` に入るときだけ番号の一致を採る。
  入らなければ同じ truename で行範囲がその行を含むフォームに付け、それも無ければ突き合わせなし
  （下の `form-path` を持たないエントリと同じく行番号でまとめる）とする。キーは形を明示し、
