...
- **`lisp-check-parens` の再配置 NOTE**: `lisp-edit-form` の要約と同じく、推定
  修正が最後のコード行より前の行にある場合は診断文にもその旨を出す
  (`format-delimiter-diagnosis` が `:repaired` の `last-code-line` で判定)。
  既知の残存: 行末の単独 `#`(`(list #` + 改行)はテキスト末尾でないため
  `:code` と分類され、読めない修正(`(list #))`)が提示されうる。
  `lisp-edit-form` 側はリーダーの失敗で偽陽性経路に入り指示を出さない。
...
- **再配置 NOTE の上限とタブ**: NOTE に列挙する行番号も `*repair-lines-limit*`
  (10)で切り「and N more」と数える(修正一覧の上限を迂回して要約が肥大しない
  ため)。インデントの比較はタブを 8 桁として見た目の幅で行う。
