...
- **`unclosed` の文面**: 「次のトップレベルフォームより前に必要」の文で示す
  閉じ記号は診断の `expected`。ただし `expected` が `]`/`}`(閉じられていない
  ...
  末尾で閉じられる形)は、案内と修正が矛盾するので案内文を出さない。この
  判定は `next-top-level-hint-line` 1 つに持ち、`lisp-check-parens` の
  `next_top_level_line` フィールドも同じ関数で決める(文が撤回した案内を
  ペイロードだけが持たない)。
...
- **再配置の注意喚起**: parinfer の閉じ括弧が置かれた行の次のコード行が同じ
  インデントにある場合(`relocating-fix-lines`、後述)、その行でフォームを
  閉じたため以降の行がそのフォームの外に出た旨を要約と診断文に明記し、入れ子の
  確認を促す。文面は `format-relocation-note` に 1 つで、`lisp-edit-form` と
  `lisp-check-parens` が同じ言葉で言う。弱いモデルが変更行だけから再配置に
  気づく必要をなくす。
...
- **閉じられていない `[`/`{` の扱いを 2 ツールで揃える**: parinfer の出力を再スキャン
  して残る唯一の指摘が `[`/`{` の開き(`expected` が `]`/`}`)なら、修復失敗
  とはせず `)` の推定修正を出す(`lisp-edit-form` が実際に書くものと同じ)。
  `unclosed` の所見には mismatch と同じ偽陽性の注意書きを添え、「修復不能」の
  断定や「missing `]`」の指示は出さない。`bracket-ambiguous-p` に基づく判定
  は「修復失敗」文にも適用する。
- **削除の位置**: 「remove N `)`」には削除する `)` の桁(`:removed-columns`)を
  添える。同じ行に複数の `)` がある場合にどれかを迷わせない。CRLF の行は
  追記でも `:append-only` にせず、桁指定の insert 形式で示す(閉じ括弧は
  `#\Return` の前に入る)。
- **未完トークンの文面**: `:outside-code` の理由が末尾の未完トークン(`#` `\`
  `'` `` ` `` `,` `@` で終わる)である場合はその旨を述べる。「文字列や
...
- **行末のリーダープレフィックスの直後には `)` を置けない**: `%any-fix-outside-code-p`
  ...
  (`(list '` + 改行)でも `(list '))` は読めないので、この規則は「挿入位置に
  `)` が続けること」として述べる。以前「既知の残存」としていた行末の `#` も
  これで塞がる。
- **行末プレフィックスの判定の精度**: `'` `` ` `` `,` `\` は無条件に却下、`@` は
  ...
  改行で終わらない `(list x bar@` の修復が却下されていた。`#` の直前として
  数える終端文字には `)` も含む(`(g x)#` は dispatch の開始であり、`)` を
  続けると「illegal sharp macro character」になる)。`]` `}` は構成文字なので
  含めない。同じ規則は削除位置にも適用する: `(defun a () 1)')` の `)` を
  消すと `'` が対象を失って読めないので却下が正しい。ただし規則は直前の
  文字しか見ないので、`(defun a () 1))'` のように削除位置の後ろに
  プレフィックスが残る形は通る(上の既知の制約と同じ種類)。
- **修復結果が空文字になる場合**: `repair-line-differences` は行数の差を
  空行で埋めて比較する。`(split-string "")` は NIL なので、`)` だけの
  テキストを parinfer が空にした場合に「remove 1 ")"」の修正が落ちていた。
- **`mismatch`(`]` を見つけた)の指示文**: 「Replace it with ")"」にも他の
  括弧判定と同じ「シンボル名の一部でなければ」の留保を付ける。`a]` に
  そのまま適用するとシンボル名を変えるだけになるため。
- **既知の制約(提示した修正が読めるとは限らない)**: `%likely-fixes` は
  parinfer の出力を釣り合いの再スキャンと `%any-fix-outside-code-p` で
  選別するが、リーダーで読み直しはしない(`(list x .` に `)` を足した
  `(list x .)` は釣り合うが読めない。実際に起きやすいのは途中で切れた
  dispatch トークンで、`(list x #b` `#p` `#5` `##` に `)` を足した結果も
  読めない。プレフィックス規則は挿入位置の直前 1 文字しか見ないため、
  `#` 単独・`#'`・`#\`・`)#` は却下できるがこれらは通す)。`lisp-edit-form`・`lisp-patch-form` は
  書く前に必ずパースするので書き込みには影響せず、影響は
  `lisp-check-parens` の提示文に限られる。修復結果をリーダーで検証して
  「釣り合うがまだ読めない」と格下げする案は、inline code では readtable が
  分からず偽の「読めない」を生むため採用せず、既知の制約として記録する。
...
- **インデントの尺度は 1 つ**: `relocating-fix-lines` は parinfer の
  ...
  持ち込むと修復と警告が食い違う。タブ混在のインデントは既知の弱点であり、
  空白でのインデントを推奨する。
- **開き括弧の曖昧さの受理規則(正確な形)**: parinfer の出力の受理は、
  「括弧をシンボル構成文字とみなした再スキャン(`*scan-brackets*` nil)が
  釣り合う」かつ「通常の再スキャンの最初の指摘が(あるとしても)`[`/`{` の
  開き側(`opener-ambiguous-p`)である」場合。`scan-delimiters` は最初の
  指摘で止まるので、開き側の指摘の先を見るために括弧なしの再スキャンが要る。
  見つかった側の `]`/`}` は従来どおり受理しない。
- **開き括弧の曖昧判定も 1 か所**: `paren-diagnostics:opener-ambiguous-p`
  (`expected` が `]`/`}`)を `lisp-edit-form` の早期拒否判定と
  `format-delimiter-diagnosis` で共有する。
- **推定修正の後に開き括弧の注意**: `[`/`{` の開きに対して `)` の推定修正を
  出す場合、修正ブロックの後に「その `[` が `(` の打ち間違いならこの修正は
  誤りで、`[` を直して再確認せよ」と添える(`lisp-edit-form` の要約も同様)。
  注意書きが修正の上にしかないと、実行可能に見える修正だけが読まれる。
- **再配置 NOTE の基準**: 修正行の次のコード行のインデントが修正行と同じ場合に
...
- **再配置 NOTE の上限**: NOTE に列挙する行番号も `*repair-lines-limit*`
  ...
  ため)。
- **回復案内文は 1 つ**: `fs-read-file` → 修正の適用 → `fs-write-file` の手順は
  `format-overwrite-recovery` に 1 つで持ち、`lisp-check-parens` の案内と
  `file-unparseable-error` の文面の両方がそれを使う(相対パス、「Likely fix」
  か「described below/above」かの切り替え、readtable の注意も含む)。
- **偽陽性時の `mismatch` の所見**: 「`]`/`}` はシンボル構成文字で自動修復
  できない」は指示の前提なので指示ブロック側に置き、偽陽性のときは所見だけを
  残す(所見に「修復できない」があると、有効なコードに修復対象があると
  主張することになる)。開き括弧の NOTE は `%repair-warning` の中で readtable
  を知った上で作り、標準構文の判定を非標準 readtable の下では出さない。
- **偽陽性時の構造化フィールド**: `false_positive` のときは `next_tool` だけを
  返し、`fix_code`・`required_args` は付けない(修正不要の入力に修正を名指し
  しない)。JSON の `likely_fixes` 各項目に `crlf` を載せ、CR を落とした
  `repaired` を書き戻さないよう伝える。
- **手作りの fix プロット**: `:append-only` が無いプロットは追記とみなさず
  修復後の行を示す(無条件の「add N `)`」は誤りになりうる)。CRLF の行では
  `giving` の修復後行を出さない(CR を落とした行を書き戻さないため)。
