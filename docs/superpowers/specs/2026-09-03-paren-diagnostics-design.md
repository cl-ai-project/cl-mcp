...
- **§5.4 の深さ計算の文脈**: `old_text`/`new_text` を単独で数えるのではなく、
  ...
  消費したテキスト(開始位置から失敗位置まで)が `)` で終わり、かつコード
  文脈の `(` を 1 つも含まない場合に限り `stray-right-parenthesis` に
  正規化する(`#S(...)` のように釣り合ったリストを読んでから失敗するマクロは
  `(` を含むので対象外)。EOF 系はそのまま。
...
- **parinfer とコメント**: parinfer はコメントを認識しないので、変更した行の
  末尾が文字列・コメント内にある修復は採用せず `:repair-failed` とする。
