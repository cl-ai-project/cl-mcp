...
> **注意: このセクションのコードは実装済みだが、その後のコードレビューで
> 大幅に修正された。最終形は `bc33b35` を参照すること。以下のコード片から
> そのままコピーしてはいけない。**
>
> レビューで判明した主な欠陥（いずれも敵対的入力ではなく通常のソースで踏める）:
>
> 1. `control-stack-exhausted` は `storage-condition` であって `error` ではないため、
>    「1件の失敗でバッチを落とさない」ための `handler-case (error ...)` を素通りしていた。
>    `serious-condition` に広げた
> 2. `%circular-p` が cdr 方向にも再帰しており、スタック深さがリストの**長さ**に比例した。
>    6万要素のクォートされたリテラルでクラッシュする。cdr スパインを反復に変更した
> 3. `level: "all"` に停止保証が無かった。`#n=` は標準のリーダー構文なのでソース自体が
>    循環しうる。入力の循環を事前に拒否する
> 4. 展開上限への到達が不動点到達と区別できなかった。`:steps-capped-p` を追加
> 5. ツール入力に対する `intern` はプロジェクト規約違反かつ無制限にシンボルが増える。
>    `find-symbol` に変更
> 6. `%read-source` が2つ目以降のフォームを黙って捨てていた。誤ったスライスが
>    「もっともらしい間違った答え」になるため、エラーにする
