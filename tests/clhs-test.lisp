...
(defparameter *clhs-load-status* :not-tried
  "Cached result of trying to load the optional :clhs library.
Avoids re-attempting a known-failing load for every test.")

(defun %clhs-available-p ()
  "Return T iff :clhs has been (or can be) loaded.
Probes once and caches the result in *clhs-load-status*."
  (when (eq *clhs-load-status* :not-tried)
    (setf *clhs-load-status*
          (handler-case
              (progn (cl-mcp/src/clhs::%ensure-clhs-loaded) :ok)
            (error () :failed))))
  (eq *clhs-load-status* :ok))

(defmacro with-clhs-or-skip (&body body)
  "Run BODY when :clhs is loadable; otherwise emit a rove SKIP marker.
Lets these tests pass on hosts where the optional :clhs library is not
installed (e.g. minimal CI Quicklisp dists)."
  `(if (%clhs-available-p)
       (progn ,@body)
       (rove:skip ":clhs library not available in this environment")))
