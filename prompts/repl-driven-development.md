...
1. **Reproduce** via `repl-eval`. On error, the backtrace in the response text lists,
   ...
   - At most 10 locals per frame are listed; the rest are counted
...
2. **Auto-expand locals**: Set `locals_preview_frames` (e.g., 3) to expand non-primitive locals
   ...
   the `inspect-object` round-trip. `locals_preview_skip_internal` (default true) skips
   CL-MCP/SBCL/ASDF infrastructure frames when counting which frames qualify.
